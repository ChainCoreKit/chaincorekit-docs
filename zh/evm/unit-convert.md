...
> 输入任一以太单位，其余单位实时联动换算；另有一档任意精度，用于 ERC-20 代币。每格都可一键复制。
...
- **手上是代币金额而不是 ETH**：比如 USDC 的原始 `1000000`，想知道它是多少个 USDC。
...
上面 11 个单位都是**以太系**的，而链上大多数金额不是以太。在**任意精度**区：

1. 填入该代币的 **decimals**（不知道的话可以用 [Token 检查器](./token-inspector.md) 查）。
   下方有 USDC / USDT（6）、WBTC（8）、DAI / WETH（18）三个快捷按钮。
2. 在 **Wei** 格填入原始的 uint256 值，或在任意精度格填入人类可读数值，两边实时互换。

改变精度**不会清空**已填的数值 —— Wei 是唯一基准，换精度只是换一种读法，
其余各格仍然自洽。
